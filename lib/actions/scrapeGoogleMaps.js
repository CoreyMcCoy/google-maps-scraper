...
export async function scrapeGoogleMaps(query) {
  try {
    ...
    console.log(`Found ${businessCards.length} business card elements using 'div.Nv2PK.THOPZb'.`);
    ...
    businessCards.each((index, element) => {
      ...
      if (detailsContainer.length === 0) {
        console.log(`Card ${index}: Skipping, could not find .lI9IFe details container.`);
    ...
    console.log(`Scraping completed. Found ${businessListings.length} valid business listings.`);
